...
; Ben Bitdiddle misunderstood the queue data representation, as the front-ptr is 
; a pointer to the front of the list/queue, it surely contains all the items the
; queue it have, but it works as a head pointer, and the rear-ptr works a tail
; pointer, which only point to the last item in the queue/list, both of them are
; represented as pointer instead of a whole data of the queue/list.
