            ...
            (let ((first-t (first-term lst))
              ...
              (let ((coeff-first (coeff first-t))
                ...
                (append (list '+) (let ([revised-coeff (if (polynomial? coeff-first)
                              ...
                              ;  (if (and (number? revised-coeff) (= revised-coeff 1) 
                              ;  (and (not (= order-first 1))(not (= order-first 0))))
                              ;      (list '+ var '^ order-first)
                              ;      (cond [(= order-first 0) (list '+ revised-coeff)]
...
(define pl-5 (make-poly 'x '(sparse (term 6 2))))
(define pl-6 (make-poly 'x '(sparse (term 1 1) (term 0 2))))
(define pl-7 (make-poly 'y '(sparse (term 2 1) (term 0 (polynomial x sparse (term 1 1) (term 0 2))))))
...
; (mul-poly pl-1 pl-4)
; (mul-poly pl-3 (mul-poly pl-1 pl-4))
...
; (mul-poly pl-1 pl-4)
; (print-poly pl-3)
; (print-poly (mul-poly pl-1 pl-4))
; (print-poly (mul-poly pl-3 (mul-poly pl-1 pl-4)))
; (print-poly pl-5)
; (print-poly pl-6)
; (print-poly (mul-poly pl-5 pl-6))
; (print-poly pl-7)
(print-poly (mul-poly pl-5 pl-7))
; '((2 * y ^ 2 + (2 * x + 4)) * x ^ 6)

; (print-poly pl-2)
; (print-poly pl-3)
; (print-poly (mul-poly pl-2 pl-3))


(define pl-8 (make-poly 'y '(sparse (term 2 2))))
(define pl-9 (make-poly 'x '(sparse (term 1 2) (term 0 4))))
(define pl-10 (make-poly 'x '(sparse (term 6 1))))
(add-poly (mul-poly pl-8 pl-10) (mul-poly pl-9 pl-10))
(print-poly (add-poly (mul-poly pl-8 pl-10) (mul-poly pl-9 pl-10)))
(mul-poly (add-poly pl-8 pl-9) pl-10)

; (map coeff (contents (term-list (mul-poly pl-2 pl-3))))
; (div '(polynomial y sparse (term 1 2) (term 0 4)) '(polynomial y sparse (term 1 1) (term 0 2)))

; (gcd 3 2)

; (define (gcd-poly list)
;   (let ((term-list ))))
